      ...
      {loading ? (
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-200 text-slate-500">
            <tr><th className="py-2">Action</th><th>Actor</th><th>Resource</th><th>Resource ID</th><th>IP</th><th>Time</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-slate-100">
                ...
                <td><button onClick={() => setSelected(selected?.id === item.id ? null : item)} className="rounded px-2 py-1 text-xs text-blue-600 hover:bg-blue-50">{selected?.id === item.id ? 'Hide' : 'View'}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
