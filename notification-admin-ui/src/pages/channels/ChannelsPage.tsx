...
export function ChannelsPage() {
  ...
  const load = () => list<Channel>('/admin/api/v1/channels').then((res) => setItems(res.data)).catch((err) => setError(err.message));
    ...
    <Panel title="Channels">
      ...
      <table className="w-full text-left text-sm">
        <thead className="border-b border-slate-200 text-slate-500">
          <tr><th className="py-2">Channel</th><th>Tenant</th><th>Status</th><th>Direction</th><th>Rate/s</th><th>Daily Quota</th><th></th></tr>
        </thead>
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="border-b border-slate-100">
              ...
              {editing === item.id ? (
                <>
                  ...
                  <td>{can('channels.update') && <button onClick={() => save(item.id)} className="text-blue-600 hover:underline">Save</button>}</td>
                </>
              ) : (
                <>
                  ...
                  <td>{can('channels.update') && <button onClick={() => startEdit(item)} className="text-blue-600 hover:underline">Edit</button>}</td>
                </>
              )}
            </tr>
          ))}
