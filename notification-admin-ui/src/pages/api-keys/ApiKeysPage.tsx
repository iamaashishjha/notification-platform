...
export function ApiKeysPage() {
  ...
  const load = () => {
    setLoading(true);
    list<ApiKey>('/admin/api/v1/api-keys' + (tenantFilter ? `?tenant_id=${encodeURIComponent(tenantFilter)}` : ''))
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => [item.name, item.tenant_name || '', item.scopes, item.status].some((value) => value.toLowerCase().includes(query)));
  }, [items, search]);
