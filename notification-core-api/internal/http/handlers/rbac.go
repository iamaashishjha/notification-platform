...
func (h Handler) ListRoles(w http.ResponseWriter, r *http.Request) {
	...
	var rows pgx.Rows
	if p.IsPlatform {
		rows, err = h.db.Query(r.Context(), `SELECT id::text, COALESCE(tenant_id::text,''), name, key, scope, status, created_at FROM roles ORDER BY created_at DESC LIMIT 100`)
	} else {
		rows, err = h.db.Query(r.Context(), `SELECT id::text, COALESCE(tenant_id::text,''), name, key, scope, status, created_at FROM roles WHERE tenant_id = $1 OR tenant_id IS NULL ORDER BY created_at DESC LIMIT 100`, p.TenantID)
	}
	if err != nil {
...
func (h Handler) ListPermissions(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.Query(r.Context(), `SELECT id::text, key, description FROM permissions ORDER BY key`)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": "query failed"})
		return
	}
